...
import type { MatchGroup } from "@/lib/generated/prisma/enums";
import type { getMatchSchedule, getTeams } from "@/actions/mpl/matches";
import type { getPlayoffMatches } from "@/actions/mpl/playoffs";
import type { getStandings } from "@/actions/mpl/standings";
...
type Schedule = Awaited<ReturnType<typeof getMatchSchedule>>;
type Standings = Awaited<ReturnType<typeof getStandings>>;
type PlayoffMatches = Awaited<ReturnType<typeof getPlayoffMatches>>;
...
type AdminManageTableProps = {
  ...
  users: AdminUser[];
  selectedWeek?: Schedule[number];
  previousWeek: Schedule[number] | null;
  nextWeek: Schedule[number] | null;
  selectedWeekCompletedMatches: number;
  playoffInitialized: boolean;
...
export function AdminManageTable({
  ...
  previousWeek,
  nextWeek,
  selectedWeekCompletedMatches,
  playoffInitialized,
  currentUserId,
}: AdminManageTableProps) {
  return (
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {playoffMatches.map((match) => (
                <div key={match.id} className="flex items-center justify-between gap-3 rounded-lg border bg-card/50 p-3">
                  ...
                  <div className="grid w-full items-center gap-2 sm:w-[420px] sm:grid-cols-[minmax(0,1fr)_32px_64px_32px_minmax(0,1fr)_24px]">
                    <div className="min-w-0 text-right">
                      <span className="block truncate text-sm font-semibold">{match.teamA?.name ?? "TBD"}</span>
                    </div>
                    <div className="flex justify-center">
                      <TeamAvatar name={match.teamA?.name ?? "TBD"} logo={match.teamA?.logo ?? null} color="left" size="small" />
                    </div>
                    <div className="text-center text-sm font-black tabular-nums">
                      {match.teamAResult ?? "-"} : {match.teamBResult ?? "-"}
                    </div>
                    <div className="flex justify-center">
                      <TeamAvatar name={match.teamB?.name ?? "TBD"} logo={match.teamB?.logo ?? null} color="right" size="small" />
                    </div>
                    <div className="min-w-0 text-left">
                ...
                {users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-right">
                      <DeleteUserButton userId={user.id} disabled={user.role === "ADMIN" || user.id === currentUserId} />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
